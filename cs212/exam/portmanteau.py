...
WORDS = readwordlist('../hw6/words.txt')
...
def natalie(words):
    ...
    for word in words:
        for i in range(1, len(word)):
            ...
            suffixes[suffix].append(prefix)

    middles = set(prefixes.keys()) & set(suffixes.keys())

    portmanteaux = []
    for m in middles:
        for s in suffixes[m]:
            for e in prefixes[m]:
                if s+m != m+e:  # avoids finding e.g. 'testest' in ['test']
                    portmanteaux.append((s+m+e, s, m, e))

    portmanteaux.sort(key=score, reverse=True)
    return portmanteaux[0][0] if portmanteaux else None

def score(pm):
    w, s, m, e = map(len, pm)
    return w - abs(s - w / 4.) - abs(m - w / 2.) - abs(e - w / 4.)
