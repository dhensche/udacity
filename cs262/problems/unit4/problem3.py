...
def csuffix(x, y):
    if len(x) == 0 or len(y) == 0:
        ...
        chart[(x, y)] = 0 if x[-1] != y[-1] else 1 + csuffix(x[:-1], y[:-1])
    return chart[(x, y)]


def prefixes(x):
    return [x[:l] for l in range(1, len(x) + 1)]


def lsubstring(x, y):
    return max(csuffix(x_prefix, y_prefix) for x_prefix in prefixes(x) for y_prefix in prefixes(y))
