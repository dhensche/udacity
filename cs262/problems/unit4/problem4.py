...
# will involve writing parsing rewrite rules (i.e., encoding a context-free
...
# element -> stmt ;
#
# The parse tree for the former is the tuple ("function",name,params,body),
...
def p_element_stmt(p):
    """element : stmt SEMICOLON
               | FUNCTION IDENTIFIER LPAREN optparams RPAREN compoundstmt"""
    p[0] = ('function', p[2], p[4], p[6]) if len(p) > 3 else ('stmt', p[1])
...
def p_stmt_if(p):
    """stmt : IF exp compoundstmt
            | IF exp compoundstmt ELSE compoundstmt"""
    p[0] = ('if-then-else', p[2], p[3], p[5]) if len(p) > 4 else ('if-then', p[2], p[3])


def p_stmt_assign(p):
    """stmt :  IDENTIFIER EQUAL exp
        | VAR IDENTIFIER EQUAL exp"""
    p[0] = ('var', p[2], p[4]) if len(p) > 4 else ('assign', p[1], p[3])


def p_stmt_3(p):
    """stmt : RETURN exp
            | exp"""
    p[0] = ('return', p[2]) if len(p) > 2 else ('exp', p[1])


def p_error(p):
    print "JavaScript Parser: Illegal character " + p.value[0]
    p.parser.skip(1)
...
jstree2 = [
    ('function', 'nobletruths', ['dukkha', 'samudaya', 'nirodha', 'gamini'], [('return', ('identifier', 'buddhism'))])]
...
mindfulness_right;
concentration_right;"""
jstree3 = [('stmt', ('var', 'view', ('identifier', 'right'))), ('stmt', ('var', 'intention', ('identifier', 'right'))),
           ('stmt', ('var', 'speech', ('identifier', 'right'))),
           ('stmt', ('assign', 'action', ('identifier', 'right'))),
           ('stmt', ('assign', 'livelihood', ('identifier', 'right'))),
           ('stmt', ('exp', ('identifier', 'effort_right'))), ('stmt', ('exp', ('identifier', 'mindfulness_right'))),
...
"""
jstree4 = [('stmt', ('if-then', ('identifier', 'cherry'), [('exp', ('identifier', 'orchard')), (
    'if-then-else', ('identifier', 'uncle_vanya'),
    [('exp', ('identifier', 'anton')), ('exp', ('identifier', 'chekov'))],
    []), ('exp', ('identifier', 'nineteen_oh_four'))]))]
