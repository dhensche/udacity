...
def filter_maker(f):
    return lambda iterable: filter(f, iterable)


def map_maker(f):
    return lambda iterable: map(f, iterable)

# We have included a few test cases. You will likely want to add your own.
numbers = [1, 2, 3, 4, 5, 6, 7]
filter_odds = filter_maker(lambda num: num % 2 == 1)
...
string_reverse_map = map_maker(lambda string: string[::-1])
# str[::-1] is cute use of the Python string slicing notation that
# reverses str. A hidden gem in the homework!
print string_reverse_map(words) == ['melohcS', 'mehcielA', 'etorw', 'eyveT', 'eht', ',namkliM', 'hcihw', 'saw',
                                    'detpada', 'otni', 'eht', 'lacisum', 'relddiF', 'no', 'eht', '.fooR']

square_map = map_maker(lambda num: num * num)
