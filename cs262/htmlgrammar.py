...
# embedded JavaScript.
#
# It is likely that the professor would explain this in class. The students
# would write the JavaScript parser as an assignment.
#
# The only trick detail is tag arguments, as in
#
# <a href="google.com" style=pretty>
#
# ... which we store as python dictionaries: {href:google.com, style:pretty}.
#
# We require well-balanced, modern HTML: <p> must be closed with </p>.
# Matching tags (e.g., <p>error</q>) will be checked later.
...
def p_tagnameend(p):
    ...
    if (tag_stack[0])[0] != p[1]:
        print "HTML Syntax Error: <" + tag_stack[0][0] + "> on line " + str(tag_stack[0][1]) + " closed by </" + str(
            p[1]) + "> on line " + str(p.lineno(1))
        exit(1)
