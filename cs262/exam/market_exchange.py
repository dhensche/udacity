...
#       [ "andreas seyfarth", "sell", "sheep" , 50 ]
#
# klaus will buy "sheep" from andreas once, at which point klaus will have
...
def evaluate(ast):
    ...
    for stmt in ast:
        if stmt[1] == 'has':
            ...
            buy.append([stmt[0], stmt[2], stmt[3]])
        elif stmt[1] == 'sell':
            sell.append([stmt[0], stmt[2], stmt[3]])

    updated = True
    while updated:
        updated = False
        for purchase in buy:
                for for_sale in sell:
                    if purchase[1:] == for_sale[1:] and env[purchase[0]] >= purchase[2]:
                        env[purchase[0]] -= purchase[2]
                        env[for_sale[0]] += purchase[2]
                        buy.remove(purchase)
                        sell.remove(for_sale)
                        updated = True
