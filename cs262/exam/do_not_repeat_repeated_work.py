...
def parse_rhs(rhs):
    if rhs[0] == "binop":
        return parse_rhs(rhs[1]) | parse_rhs(rhs[3])
...
def optimize(ast):
    ...
    for branch in ast:
        reassigned = False
        for (key, assignment) in safe.items():
            # if the current value we are assigning to is contained in a reassignment then remove it from the safe re-assignment list because the value has changed
            if branch[1] in parse_rhs(assignment):
                safe.pop(key)
            # if the right hand sides match then we can update the statement to just point to the other var
            elif branch[2] == assignment:
                reassigned = True
                result.append(("assign", branch[1], ("identifier", key)))
                break
